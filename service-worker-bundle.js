    ...
    (function(root) {
      ...
      var encode = function(string, options) {
        ...
        if (encodeEverything) {
          ...
          if (useNamedReferences) {
            string = string.replace(regexEncodeNonAscii, function(string3) {
              return "&" + encodeMap[string3] + ";";
            });
          }
        } else if (useNamedReferences) {
          if (!allowUnsafeSymbols) {
            string = string.replace(regexEscape, function(string3) {
              return "&" + encodeMap[string3] + ";";
            });
          }
...
var LocalDB = class _LocalDB {
  constructor() {
    if (_LocalDB.instance) return _LocalDB.instance;
    this.dbName = "GmailExtensionDB";
    this.dbVersion = 1;
    this.db = null;
    _LocalDB.instance = this;
  }
  async openDB() {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        const createStore = (name2) => {
          if (!db.objectStoreNames.contains(name2)) {
            console.log(`Creating object store: ${name2}`);
            db.createObjectStore(name2, { keyPath: "id" });
          }
        };
  ...
  async getPredPriority(emailID) {
    ...
    return new Promise((resolve, reject) => {
      ...
      getRequest.onsuccess = (event) => {
        ...
        if (result && result.priority !== void 0) {
...
var DELAY_MS = 250;
var VOCAB_LEN = 1e3;
async function listEmails(counts, accessToken, train2) {
...
async function getVocab(numEmails, token2) {
  console.log("In fetch vocab emails");
  try {
    console.log("Token: ", token2);
    console.log("NUM EMAILS: ", numEmails);
    const emailIds = await listEmails(numEmails, token2, false);
    if (!emailIds || emailIds.length === 0) {
      console.log("No emails found.");
      return [];
    }
    console.log("Length: ", emailIds.length);
    const emails = await fetchEmailData(emailIds, token2);
    ...
    const sorted = Object.entries(freqMap).sort((a, b) => b[1] - a[1]).slice(0, VOCAB_LEN).map(([word]) => word);
...
async function fetchTrainingEmails(numEmails, token2, train2) {
  console.log("In fetch training emails");
  try {
    console.log("Token: ", token2);
    console.log("NUM EMAILS: ", numEmails);
    const emailIds = await listEmails(numEmails, token2, train2);
    if (!emailIds || emailIds.length === 0) {
      console.log("No emails found.");
      return [];
    }
      ...
      if (train2) {
        if (emailObj.id && await dbs.checkEmailExists(emailObj.id)) {
          ...
          if (predPriority && predPriority === 1) {
            ...
            await removePredictionLabel(emailObj.id, token2);
          }
...
async function fetchInboxEmails(numEmails, token2) {
  ...
  try {
    ...
    console.log("Token: ", token2);
    console.log("NUM EMAILS: ", numEmails);
    let emailIds = await listEmails(numEmails, token2, train2);
    if (!emailIds || emailIds.length === 0) {
      console.log("No emails found.");
      return [];
    }
...
function processEmail(email) {
  ...
  if (payload.body?.data) {
    ...
    content = convert(html, { ignoreHref: true });
    content = import_he.default.decode(content);
...
async function getDBEmails(tableName) {
  const emails = await dbs.getAllEmails(tableName);
  return emails;
}
async function clearDBEmails(tableName) {
  await dbs.clearEmails(tableName);
}
async function saveEmailPriority(emailId, isPriority) {
  await dbs.updatePriority(emailId, isPriority);
}
async function saveToPredictedDB(emailID, prediction) {
  await dbs.saveToPredicted(emailID, prediction);
}

// service-worker.js
var emailsStored = false;
var trainingSignal = false;
var isTraining = false;
var volatileToken = null;
var API_BASE = "https://email-priority-filter-backend.onrender.com";
chrome.runtime.onMessage.addListener(async function(message, sender, sendResponse2) {
...
async function getAccessToken() {
  ...
  let token2 = await fetchToken(uuid);
  if (!token2) {
    console.log("Token not found, starting auth flow");
    await authFlow();
    token2 = await fetchToken(uuid);
  }
  if (token2) {
    volatileToken = token2;
...
async function fetchToken(uuid) {
  try {
    ...
    console.error("Error fetching token:", e2);
    return null;
  }
}
chrome.runtime.onMessage.addListener((request, sender, sendResponse2) => {
  if (request.action === "fetchEmails") {
    (async () => {
      ...
      const { vocabSaved, trainingSignal: trainingSignal2, isTraining: isTraining2 } = await chrome.storage.local.get([
        "vocabSaved",
        "trainingSignal",
        "isTraining"
      ]);
      if (vocabSaved && trainingSignal2 && !isTraining2) {
        train();
      }
    })();
    return true;
...
chrome.runtime.onMessage.addListener(async function(request, sender, sendResponse2) {
  if (request.action === "initiateTraining") {
    sendResponse2({ message: "Received signal to initiate training." });
    chrome.storage.local.set({ trainingSignal: true });
    const { vocabSaved, emailsStored: emailsStored2, isTraining: isTraining2 } = await chrome.storage.local.get([
      "vocabSaved",
      "emailsStored",
      "isTraining"
    ]);
    console.log("Vocab saved: ", vocabSaved);
    console.log("Emails stored: ", emailsStored2);
    console.log("Training signal ", trainingSignal);
    console.log("isTraining: ", !isTraining2);
    if (vocabSaved && emailsStored2 && !isTraining2) {
      train();
    }
    return true;
  }
    ...
    if (firstTrain === void 0 && i === 0) {
      ...
      const response = await fetch(`${API_BASE}/start_train`, {
        ...
        body: JSON.stringify({
          uiud,
          preferences,
          // pass your preferences object here
          emails: batch
        })
      });
      if (!response.ok) {
...
chrome.runtime.onMessage.addListener(async function(request, sender, sendResponse2) {
  if (request.action === "savePriority") {
    ...
    sendResponse2({ message: "Received the priority status.", emailId: request.emailId, isPriority: request.isPriority });
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse2) => {
  if (request.action === "savePreferences") {
    ...
    sendResponse2({ message: "Received the pref.", preference: pref });
    console.log("Saving preferences:", pref);
    chrome.storage.local.set({ preferences: pref });
  }
});
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse2) {
  if (request.action === "saveVocab") {
    (async () => {
      ...
      const { emailsStored: emailsStored2, trainingSignal: trainingSignal2, isTraining: isTraining2 } = await chrome.storage.local.get([
        "emailsStored",
        "trainingSignal",
        "isTraining"
      ]);
      if (emailsStored2 && trainingSignal2 && !isTraining2) {
        train();
      }
    })();
    return true;
...
async function predictPriority() {
  ...
  for (let i = 0; i < emails.length; i += batchSize) {
    ...
    for (let i2 = 0; i2 < batchIDs.length; i2++) {
      await saveToPredictedDB(batchIDs[i2], predictions[i2]);
      if (predictions[i2] == 1) {
        await displayPrediction(batchIDs[i2], access_token);
      }
    }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse2) => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          sendResponse2({ success: false, message: "Unable to erase data." });
        } else {
          console.log("Successfully erased data on server.");
          sendResponse2({ success: true, message: "Successfully erased data." });
        }
      } catch (err) {
        console.error("Failed to process eraseData:", err);
        sendResponse2({ success: false, error: err.message });
      }
    })();
    return true;
  }
